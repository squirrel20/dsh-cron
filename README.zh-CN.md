...
- 运行会话是普通会话，宿主不再拒收发给它的消息：运行进行中打开该会话并输入，会给无人值守的运行插入一条用户消息（可能出现在该轮 summary 里），运行结束时该会话视图同样会退回工作区。
- 运行会话不再从工作区侧边栏隐藏（侧边栏只隐藏 `origin: "subagent"` 会话），每次运行（进行中或已结束）都会列出；高频作业会累积大量条目，已结束的运行也可从侧边栏续聊，此时不带 `[CRON RUN]` framing。
