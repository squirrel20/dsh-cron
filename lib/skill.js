...
1. **Collect the requirement.** What should run (an agent prompt or an exec-style command), on what schedule, and under what kebab-case name. Prompts for agent tasks must stand alone: the headless run has none of this conversation's context.
...
6. **Undo / remove.** \`cron_delete\` removes a manual job and its run history (refused while a run is in flight). Config-declared jobs are read-only here — point the user at the profile config for those.

## Spec vocabulary

- \`name\` — required, must match \`^[a-z][a-z0-9-]*$\`, unique across all jobs.
...
- The other cron tools also work on config jobs: \`cron_enable\`/\`cron_disable\` toggle dispatch, \`cron_run_now\` fires a manual run.
`;
