/**
 * dsh-cron — task executors.
 *
 * The dependency-free half: `command` tasks (spawned child processes with a
 * bounded output tail), `callback` tasks (a plugin-registered handler run in
 * process), delivery, and the pure text folds shared with the agent
 * executor. The `agent` executor lives in ./agent-task.js because it
 * imports dsh host packages — the service loads it lazily so the scheduling
 * core stays testable without a mounted harness. Executors return a
 * settlement `{status, summary, ...}` — they never throw for task failure,
 * only for executor bugs.
 * @module dsh-cron/executor
 */
import { spawn } from "node:child_process";

/** Bound on stored summaries/output tails; storage is a ledger, not a log sink. */
export const SUMMARY_LIMIT = 8_192;

/** Keep the TAIL of oversized text — the end carries the outcome. */
export function tail(text, limit = SUMMARY_LIMIT) {
	return text.length <= limit ? text : `…${text.slice(text.length - limit + 1)}`;
}

/**
 * Iterate the owned event interval `[firstSeq, session.seq)`. dsh ≥ 0.1.5
 * (session format V3) exposes events only through `session.eventAt(seq)`;
 * older hosts expose a `session.events` array. A plain array is accepted too
 * (tests, callers that already materialized the events).
 */
export function* ownedEvents(source, firstSeq) {
	if (typeof source?.eventAt === "function") {
		const length = source.seq;
		for (let seq = firstSeq; seq < length; seq++) {
			const event = source.eventAt(seq);
			if (event === undefined) throw new Error(`cron summary cannot read seq ${String(seq)} below captured length ${String(length)}`);
			yield event;
		}
		return;
	}
	const events = Array.isArray(source) ? source : source?.events ?? [];
	for (const event of events) {
		if (event.seq < firstSeq) continue;
		yield event;
	}
}

/**
 * Last non-empty assistant text and final turn outcome in one owned event
 * interval (same fold as dsh-headless). `source` is the agent's Session (or
 * an event array, see {@link ownedEvents}).
 */
export function summarize(source, firstSeq) {
	let started = false;
	let text = "";
	let reason;
	for (const event of ownedEvents(source, firstSeq)) {
		if (event.type === "turn/start") {
			started = true;
			continue;
		}
		if (!started) continue;
		if (event.type === "assistant/message") {
			const joined = event.data.message.content
				.filter((block) => block.type === "text")
				.map((block) => block.text)
				.join("");
			if (joined !== "") text = joined;
		}
		if (event.type === "turn/end") reason = event.data.reason;
	}
	return { text, reason };
}

/**
 * Fixed framing so the model knows this is unattended: no user to ask.
 * Registered as a scoped system-prompt section by the agent executor; the
 * user message then carries only the author's prompt. The harness renders
 * sections with strict `{{variable}}` interpolation and the job name is
 * user input, so any `{{` pair in it is broken up.
 */
export function renderCronFraming(job, targetIso) {
	return [
		"[CRON RUN]",
		`job: ${JSON.stringify(job.name).replaceAll("{{", "{ {")}`,
		`scheduled_for: ${targetIso}`,
		"This is an unattended scheduled run. No user is watching and nobody can answer questions — never ask, never wait for confirmation. Do the task, then end with a concise report of what happened.",
	].join("\n");
}

/** Fallback when the host lacks the system-prompt service: framing rides the user message. */
export function renderCronPrompt(job, targetIso) {
	return [renderCronFraming(job, targetIso), "", job.task.prompt].join("\n");
}

/**
 * Durable Session header for one agent run.
 *
 * Deliberately plain — no `origin`. The harness reads `origin: "subagent"` as
 * "child of a durable parent" and then steers that Session away from the
 * ordinary address for good (`session/agent-busy`: "subagent Sessions require
 * their durable parent address"), while a scheduled run has no parent to be
 * addressed through: its run row could never reopen the finished
 * conversation. `dsh-headless` persists the same plain header for its
 * one-shot runs.
 * @param cwd - the job's working directory; empty means the scheduler's own.
 * @returns the `meta` argument for `ctx.agents.create`.
 */
export function runSessionMeta(cwd) {
	return { cwd: cwd !== "" ? cwd : process.cwd() };
}

/**
 * Run one command task to settlement.
 * @param control - cancellation seam; teardown kills the child process.
 * @param onOutput - optional live-tail observer, called with the bounded
 * output tail on every chunk; feeds the web overlay's streaming view and
 * never affects settlement.
 * @returns `{status: "ok"|"failed"|"timeout", summary, exitCode?, error?}`
 */
export function runCommandTask(job, control, onOutput) {
	return new Promise((resolve) => {
		const [cmd, ...args] = job.task.argv;
		const child = spawn(cmd, args, {
			cwd: job.task.cwd !== "" ? job.task.cwd : process.cwd(),
			stdio: ["ignore", "pipe", "pipe"],
		});
		control?.register(() => {
			child.kill("SIGKILL");
		});
		let output = "";
		let settled = false;
		const collect = (chunk) => {
			output = tail(output + chunk.toString(), SUMMARY_LIMIT * 2);
			onOutput?.(tail(output));
		};
		child.stdout.on("data", collect);
		child.stderr.on("data", collect);
		const timer = setTimeout(() => {
			child.kill("SIGTERM");
			setTimeout(() => child.kill("SIGKILL"), 10_000).unref();
			settle({
				status: "timeout",
				summary: tail(output),
				error: `command exceeded ${job.task.timeoutSeconds}s`,
			});
		}, job.task.timeoutSeconds * 1000);
		const settle = (result) => {
			if (settled) return;
			settled = true;
			clearTimeout(timer);
			resolve(result);
		};
		child.on("error", (error) => {
			settle({ status: "failed", summary: tail(output), error: String(error) });
		});
		child.on("close", (code) => {
			settle({
				status: code === 0 ? "ok" : "failed",
				summary: tail(output),
				exitCode: code ?? -1,
				error: code === 0 ? undefined : `exit code ${code}`,
			});
		});
	});
}

/**
 * Run one callback task to settlement: a plugin-registered handler executed
 * in this process, with the same timeout, cancellation and settlement
 * vocabulary a spawned command gets.
 *
 * Why this exists: a provider plugin that registers a job usually wants to
 * call its own code, and the alternatives are all worse — shelling out to
 * `curl` needs the host's own URL, and spawning a script duplicates logic
 * that already sits one import away. Only plugin registrations can carry a
 * handler, so a config or manual job can never name one.
 *
 * The handler receives `{ job, target, seq, signal }` and may return a
 * summary string, or `{ ok?, summary?, error? }`. Anything thrown settles
 * the run as `failed` — the handler owns its own error text. Cancellation
 * and timeout abort the signal, but a handler that ignores it only loses its
 * result: the run settles regardless, and nothing waits on the straggler.
 * @param control - cancellation seam; teardown aborts the handler's signal.
 * @returns `{status: "ok"|"failed"|"timeout", summary, error?}`
 */
export async function runCallbackTask(job, control, handler, { target, seq } = {}) {
	const controller = new AbortController();
	control?.register(() => controller.abort());
	let timer;
	const timeout = new Promise((resolve) => {
		timer = setTimeout(() => {
			controller.abort();
			resolve({
				status: "timeout",
				summary: "",
				error: `callback exceeded ${job.task.timeoutSeconds}s`,
			});
		}, job.task.timeoutSeconds * 1000);
	});
	const invoke = (async () => {
		try {
			const value = await handler({ job, target, seq, signal: controller.signal });
			if (typeof value === "string") return { status: "ok", summary: tail(value) };
			const ok = value?.ok ?? true;
			return {
				status: ok ? "ok" : "failed",
				summary: tail(String(value?.summary ?? "")),
				...(value?.error === undefined ? {} : { error: String(value.error) }),
			};
		} catch (error) {
			return { status: "failed", summary: "", error: String(error) };
		}
	})();
	try {
		return await Promise.race([invoke, timeout]);
	} finally {
		clearTimeout(timer);
	}
}

/**
 * Feed one settled run record to the job's delivery command via stdin.
 * Delivery failure is logged by the caller, never escalated: the run itself
 * already settled.
 */
export function deliver(job, record) {
	return new Promise((resolve) => {
		const [cmd, ...args] = job.delivery.argv;
		const child = spawn(cmd, args, { stdio: ["pipe", "ignore", "pipe"] });
		let stderr = "";
		child.stderr.on("data", (chunk) => {
			stderr = tail(stderr + chunk.toString(), 2_048);
		});
		const timer = setTimeout(() => {
			child.kill("SIGKILL");
			resolve({ ok: false, error: "delivery command exceeded 60s" });
		}, 60_000);
		child.on("error", (error) => {
			clearTimeout(timer);
			resolve({ ok: false, error: String(error) });
		});
		child.on("close", (code) => {
			clearTimeout(timer);
			resolve(code === 0 ? { ok: true } : { ok: false, error: `exit ${code}: ${stderr}` });
		});
		child.stdin.end(JSON.stringify(record));
	});
}
