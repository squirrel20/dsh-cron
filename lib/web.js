 ...
 *
 * Four exact HTTP routes on the shared `ctx.webServer`. The read
 * (`GET /dsh-cron/api/state`) answers the sidebar overlay with the same
 * views the conversational tools read. The four writes (`POST …/run-now`,
 * `POST …/stop`, `POST …/jobs`, `POST …/delete`) steer the scheduler through the service's
 * own methods; each demands an `application/json` body, so a cross-site
...
/** The three write actions, keyed by route path. */
