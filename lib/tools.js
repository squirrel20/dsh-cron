...
const LIST_DESCRIPTION =
	"List every host-plane cron job: task kind, schedule, effective enabled state (profile config or a conversational override), whether a run is in flight, the next occurrence (UTC), and the latest run outcome. source 'config' jobs are declared in profile config (read-only here); source 'manual' jobs were created at runtime and can be removed with cron_delete; source 'plugin' jobs were registered by the plugin named in owner — they can be toggled and run, but only that plugin (or uninstalling it) changes or removes them, and orphan true marks one whose plugin is no longer mounted, leaving only its history to clear with cron_delete. Use cron_create to add a manual job, cron_enable/cron_disable to toggle dispatch, and cron_run_now to trigger one manually.";
