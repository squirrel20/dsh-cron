...
const RUNS_DESCRIPTION =
	"Read recent cron run records, newest first: status (ok|failed|timeout|skipped-overlap|replaced|aborted|running), the occurrence answered, a bounded summary (agent's final text or command output tail), the persisted sessionId for agent runs, and manual true for cron_run_now triggers. Optionally filter to one job; history is bounded per job.";
