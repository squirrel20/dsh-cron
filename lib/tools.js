 ...
 * declarative (created and deleted only in profile config), plugin-registered
 * jobs belong to their provider package (see ../registry.js); manual jobs are
...
const JOB_VIEW_SCHEMA = {
	...
	properties: {
		...
		kind: { type: "string", required: true, enum: ["agent", "command", "callback"] },
		schedule: { type: "string", required: true },
		enabled: { type: "boolean", required: true },
		enabledSource: { type: "string", required: true, enum: ["config", "override"] },
		source: { type: "string", required: true, enum: ["config", "manual", "plugin"] },
		owner: { type: "string" },
		orphan: { type: "boolean" },
...
const LIST_DESCRIPTION =
	"List every host-plane cron job: task kind, schedule, effective enabled state (profile config or a conversational override), whether a run is in flight, the next occurrence (UTC), and the latest run outcome. source 'config' jobs are declared in profile config (read-only here); source 'manual' jobs were created at runtime and can be removed with cron_delete; source 'plugin' jobs were registered by the plugin named in owner (kind 'callback' means the work is that plugin's own in-process handler, not a prompt or a command) — they can be toggled and run, but only that plugin (or uninstalling it) changes or removes them, and orphan true marks one whose plugin is no longer mounted, leaving only its history to clear with cron_delete. Use cron_create to add a manual job, cron_enable/cron_disable to toggle dispatch, and cron_run_now to trigger one manually.";
...
const DELETE_DESCRIPTION =
	"Delete one manual cron job together with its entire run history (releasing its headless session files to garbage collection). Only jobs created at runtime (source 'manual' in cron_list) can be deleted — profile-config jobs fail with config_job and must be removed in profile config, plugin-registered jobs fail with plugin_job and are removed by uninstalling their plugin; a job with a run in flight fails with already_running. The one exception is an orphan plugin job (orphan true): its plugin is gone, so deleting it just clears the leftover history. Deletion is permanent: confirm with the user first.";
...
export function registerCronTools(rootCtx, toolCtx, agent, getService) {
	...
	try {
		...
		disposers.push(toolCtx.tools.register(defineTool({
			...
			parameters: {
				name: {
					type: "string",
					required: true,
					description: "Job name matching ^[\\p{L}\\p{N}][\\p{L}\\p{N}_-]*$ (letters incl. non-Latin, digits, - and _; no spaces), unique across all jobs (check cron_list first).",
				},
				description: {
					type: "string",
					description: "Short human-readable one-liner saying what this job does; shown in cron_list and the web overlay.",
				},
		...
		disposers.push(toolCtx.tools.register(defineTool({
			...
			parameters: {
				job: { type: "string", required: true, description: "Exact job name from cron_list; must have source 'manual', or source 'plugin' with orphan true." },
			},
