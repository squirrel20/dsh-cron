...
import { deliver, runCallbackTask, runCommandTask } from "./executor.js";
...
/** Session ids must be path-safe (`[A-Za-z0-9_-]`, dsh ≥ 0.1.5 keys the
 * session projection cache per record by id); a job name is user text. */
const SAFE_ID_SEGMENT = /^[A-Za-z0-9][A-Za-z0-9_-]*$/;

/**
 * The job-name segment of a cron session id. ASCII-safe names pass through;
 * anything else keeps its safe characters (if any) and gains a short stable
 * digest so the id stays readable-ish, unique per job, and path-safe.
 */
export function cronSessionSlug(jobName) {
	if (SAFE_ID_SEGMENT.test(jobName)) return jobName;
	const ascii = jobName.replace(/[^A-Za-z0-9_-]+/g, "").replace(/^[_-]+|[_-]+$/g, "");
	const digest = createHash("sha256").update(jobName).digest("hex").slice(0, 8);
	return `${ascii === "" ? "job" : ascii}-${digest}`;
}

/** Mint the session id for one agent run: `cron-<slug>-<uuid>`. */
export function newCronSessionId(jobName) {
	return `cron-${cronSessionSlug(jobName)}-${randomUUID()}`;
}
 ...
 * @param options.allowCallback - accept `task.kind: "callback"`, whose work is
 *   an in-process handler rather than a prompt or an argv. Only plugin
 *   registrations pass this: config and manual jobs are specs on disk with
 *   nobody to supply the function, so accepting the kind there would mint a
 *   job that can never run.
 */
function normalizeJob(raw, { allowCallback = false } = {}) {
	...
	const kinds = allowCallback ? ["agent", "command", "callback"] : ["agent", "command"];
	if (!kinds.includes(kind)) {
		throw new CronInputError(
			`job '${jobName}': task.kind must be ${kinds.map((k) => `'${k}'`).join(" or ")}`
			+ (kind === "callback" ? " ('callback' is only available to plugin-registered jobs)" : ""),
		);
	}
...
/** Normalize options for plugin-registered jobs: they alone may carry a handler. */
export const PLUGIN_NORMALIZE = { allowCallback: true };

/** Validate the whole job list; duplicate names are a config bug. */
export function normalizeJobs(rawJobs, options) {
	const jobs = rawJobs.map((raw) => normalizeJob(raw, options));
	const seen = new Set();
...
/**
 * Counting semaphore bounding concurrent run execution host-wide. `Infinity`
 * slots (the default) makes every acquire fall through — free stays Infinity
 * under both -1 and +1, so no run ever queues behind an unrelated job.
 */
class Gate {
	constructor(slots) {
		this.free = slots;
...
export class CronService {
	...
	constructor(ctx, config, jobs, gcConfig) {
		this.ctx = ctx;
		// A private copy: start() merges manual and plugin jobs into this list,
		// and the caller's array outlives the service (the cordis effect can
		// rebuild the service around the same config-derived jobs).
		this.jobs = [...jobs];
		this.historyLimit = Math.max(1, config.historyLimit);
		this.gate = new Gate(config.maxConcurrentRuns > 0 ? config.maxConcurrentRuns : Infinity);
	...
	/**
	 * Boot repair of the durable ledger: forget jobs nobody declares any more,
	 * and settle runs the previous host process left mid-flight.
	 *
	 * The known-set is deliberately wider than `this.jobs`: a plugin job is
	 * absent from that list until its provider registers, which races this
	 * boot, so its names come from the durable `plugin` projection instead.
	 * Without that, a provider mounting a second late would find its run
	 * history already swept as removed-from-config — and, with it, its session
	 * files released to the GC.
	 */
	async repairLedger() {
		const runs = this.domain.table("runs");
		const known = new Set([...this.jobs.map((job) => job.name), ...this.domain.table("plugin").keys()]);
		...
		entry.promise = (async () => {
			...
			try {
				...
				try {
					...
					} else if (job.task.kind === "callback") {
						result = await runCallbackTask(job, control, job.handler, { target: targetIso, seq });
					} else {
	...
	/**
	 * cron_list: every declared job, config order, followed by the orphan rows
	 * of plugin jobs whose provider is not mounted (see {@link orphanViews}).
	 */
	listView() {
		const now = Date.now();
		return [...this.jobs.map((job) => this.jobView(job, now)), ...this.orphanViews()];
	}

	/** cron_runs: recent run records, newest first, optionally one job. */
	runsView(jobName, limit) {
		// Orphan plugin jobs answer too: their provider is gone, their history
		// is not, and reading it is how the user decides whether to clear it.
		if (
			jobName !== undefined
			&& !this.jobs.some((job) => job.name === jobName)
			&& this.domain.table("plugin").get(jobName) === undefined
		) {
			return this.notFound(jobName);
		}
		const runs = this.domain.table("runs");
		let keys = [...runs.keys()];
	...
	/**
	 * Plugin mode: attach one job registered by a provider plugin (see
	 * ./registry.js). Called on the registry's serial queue, both when a
	 * provider mounts and when this service replaces an earlier one, so it is
	 * idempotent by construction: an already-attached job of the same name is
	 * replaced in place, keeping its dispatch state and run history.
	 *
	 * Precedence is config > plugin > manual. Config wins because the profile
	 * is the most explicit statement of intent (the registry refuses such a
	 * name up front; this branch only catches a config reload racing an
	 * attach). Plugin wins over manual because a package's schedules come with
	 * the scripts they drive — the shadowed manual row stays on disk, so
	 * uninstalling the provider brings the user's own job back.
	 * @param rawSpec - the provider's raw job spec, re-normalized here.
	 * @param owner - provider identity, shown in the overlay.
	 * @param handler - required for a `callback` task: the function one
	 *   occurrence runs. It lives only in memory — the provider's code is the
	 *   truth, so a handler is never persisted and an orphan callback job is
	 *   inert by construction.
	 */
	async attachPluginJob(rawSpec, owner, handler) {
		let job;
		try {
			job = normalizeJobs([rawSpec], PLUGIN_NORMALIZE)[0];
		} catch (error) {
			this.ctx.logger.error(`cron: plugin job from ${owner} does not normalize, not scheduling it: ${String(error)}`);
			return { code: "invalid_job", message: String(error) };
		}
		const name = job.name;
		if (job.task.kind === "callback" && typeof handler !== "function") {
			this.ctx.logger.error(`cron: plugin job '${name}' from ${owner} is a callback task without a handler; not scheduling it`);
			return { code: "invalid_job", message: `job '${name}': a callback task needs a handler` };
		}
		const existing = this.jobs.find((j) => j.name === name);
		const existingSource = existing === undefined ? undefined : existing.source ?? "config";
		if (existingSource === "config") {
			this.ctx.logger.error(`cron: plugin job '${name}' from ${owner} collides with a config job; not scheduling it`);
			return { code: "config_job", message: `job '${name}' is declared in profile config` };
		}
		if (existingSource === "plugin" && existing.owner !== owner) {
			this.ctx.logger.error(`cron: plugin job '${name}' is already registered by ${existing.owner}; ${owner} not scheduling it`);
			return { code: "job_exists", message: `job '${name}' is already registered by ${existing.owner}` };
		}
		if (existingSource === "manual") {
			this.ctx.logger.warn(
				`cron: plugin job '${name}' from ${owner} shadows a manual job of the same name; `
				+ "the manual copy stays on disk but is not scheduled",
			);
		}
		job.source = "plugin";
		job.owner = owner;
		if (handler !== undefined) job.handler = handler;
		const nowIso = new Date().toISOString();
		const table = this.domain.table("plugin");
		const kept = table.get(name);
		await table.put(name, {
			owner,
			spec: rawSpec,
			createdAt: kept?.createdAt ?? nowIso,
			lastSeenAt: nowIso,
		});
		const jobsTable = this.domain.table("jobs");
		let state = jobsTable.get(name);
		if (state === undefined) {
			state = { anchorMs: Date.now(), lastFiredMs: 0, runSeq: 0 };
			await jobsTable.put(name, state);
		}
		this.jobs = existing === undefined
			? [...this.jobs, job]
			: this.jobs.map((j) => (j.name === name ? job : j));
		// Same catch-up rule config jobs get at boot: a misfire:runOnce job
		// reaches back to its last dispatched occurrence, everything else
		// starts from now. A provider remount is exactly the "host was down"
		// case that rule exists for.
		const floor = job.policy.misfire === "runOnce" && state.lastFiredMs > 0 ? state.lastFiredMs : Date.now();
		this.floor.set(name, floor);
		this.wake();
		return { job: this.jobView(job) };
	}

	/**
	 * Plugin mode: the provider unmounted. The job stops being scheduled and a
	 * run in flight is cancelled (the code behind it is going away), but the
	 * durable state stays: the `plugin` row, the dispatch state and the whole
	 * run history survive, so reinstalling the provider resumes the same job
	 * rather than starting a stranger with the same name. What is left behind
	 * shows in the overlay as an orphan row the user can delete.
	 */
	detachPluginJob(jobName, owner) {
		const job = this.jobs.find((j) => j.name === jobName);
		if (job === undefined || (job.source ?? "config") !== "plugin" || job.owner !== owner) return false;
		this.running.get(jobName)?.control.cancel("aborted");
		this.pending.delete(jobName);
		this.jobs = this.jobs.filter((j) => j.name !== jobName);
		this.floor.delete(jobName);
		this.ctx.logger.info(`cron: plugin job '${jobName}' detached (${owner} unmounted); its history is kept`);
		this.wake();
		return true;
	}

	/**
	 * Views for plugin rows whose provider is not mounted: the job cannot run,
	 * but its history is still on disk and the user needs a way to see and
	 * clear it. Shaped like a jobView with `orphan: true` and no next
	 * occurrence; a row whose stored spec no longer normalizes is skipped from
	 * the list (deleteJob still reaches it by name).
	 */
	orphanViews() {
		const out = [];
		const runs = this.domain.table("runs");
		for (const [name, record] of this.domain.table("plugin").entries()) {
			if (this.jobs.some((job) => job.name === name)) continue;
			let job;
			try {
				job = normalizeJobs([record.spec], PLUGIN_NORMALIZE)[0];
			} catch {
				continue;
			}
			const view = {
	...
	/** Refusal for a name that only exists as an orphan plugin row — the reason matters more than the code. */
	notFound(jobName) {
		const orphan = this.domain.table("plugin").get(jobName);
		return {
			code: "job_not_found",
			message: orphan === undefined
				? `no cron job named '${jobName}'`
				: `cron job '${jobName}' was registered by ${orphan.owner}, which is not mounted; only its history remains`,
		};
	}
	...
	async addJob(rawSpec) {
		...
		// An orphan plugin row still owns its ledger under this name; letting a
		// manual job take it would silently inherit a stranger's run history.
		const orphan = this.domain.table("plugin").get(job.name);
		if (orphan !== undefined) {
			return {
				code: "job_exists",
				message: `cron job '${job.name}' still holds the history of the same job registered by ${orphan.owner}; delete it first`,
			};
		}
		job.source = "manual";
	...
	async updateJob(jobName, rawSpec) {
		...
		const source = existing.source ?? "config";
		if (source === "plugin") {
			return {
				code: "plugin_job",
				message: `job '${jobName}' is registered by the plugin ${existing.owner}; edit it in that plugin's config`,
			};
		}
		if (source !== "manual") {
	...
	async deleteJob(jobName) {
		const job = this.jobs.find((j) => j.name === jobName);
		if (job === undefined) {
			// The one deletion that works without a live job: an orphan plugin
			// row, whose provider is gone and whose ledger only the user can
			// decide to clear.
			if (this.domain.table("plugin").get(jobName) !== undefined) return this.purgeJob(jobName);
			return this.notFound(jobName);
		}
		if ((job.source ?? "config") === "plugin") {
			return {
				code: "plugin_job",
				message: `job '${jobName}' is registered by the plugin ${job.owner}; unmount that plugin to remove it`,
			};
		}
		...
		this.jobs = this.jobs.filter((j) => j.name !== jobName);
		this.floor.delete(jobName);
		return await this.purgeJob(jobName);
	}

	/** Drop one job's every durable trace: spec rows, dispatch state, whole ledger. */
	async purgeJob(jobName) {
		await this.domain.table("manual").delete(jobName);
		await this.domain.table("plugin").delete(jobName);
