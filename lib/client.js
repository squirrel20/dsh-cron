window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			".dshCron_form{display:flex;flex-direction:column;gap:10px;min-width:min(360px,72vw)}",
			...
			return h(react.Fragment, {},
				h(primitives.Modal, {
					...
					h("div", { className: "dshCron_form" },
						...
						taskKind === "agent" ? h("div", { className: "dshCron_inputRow" },
							field(t("form.preset"), h("select", {
								...
								h("option", { value: "" }, (() => {
									const id = options?.defaults?.preset;
									const name = options?.presets?.find((entry) => entry.id === id)?.name ?? id;
							...
							field(t("form.access"), h("select", {
								...
								h("option", { value: "" }, (() => {
									const id = options?.defaults?.access;
									const name = options?.access?.find((entry) => entry.id === id)?.name ?? id;
									return name !== undefined ? t("form.inherit", { name }) : t("form.inherit.bare");
								})()),
								...(options?.access ?? []).map((entry) => h("option", { key: entry.id, value: entry.id, title: entry.description }, entry.name))))) : null,
