window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			".dshCron_delConfirm{flex:none;font-size:11px;line-height:18px;color:var(--dsw-alias-state-error-primary);cursor:pointer;background:0 0;border:0;border-radius:5px;padding:1px 6px;font-family:inherit}",
			".dshCron_delConfirm:hover{background:var(--dsw-alias-interactive-bg-hover-danger,var(--dsw-alias-interactive-bg-hover))}",
		...
		const en = {
			...
			"detail.delete.confirm": "Confirm delete",
			"time.today": "{time}",
		...
		*/
		...
		function IconTrash() {
			return h("svg", { width: 13, height: 13, viewBox: "0 0 14 14", fill: "none", "aria-hidden": "true" },
				h("path", { d: "M2.3 3.9h9.4M5.6 3.9V2.6c0-.4.3-.7.7-.7h1.4c.4 0 .7.3.7.7v1.3m1.9 0-.4 7c0 .7-.6 1.2-1.3 1.2H5.4c-.7 0-1.3-.5-1.3-1.2l-.4-7", stroke: "currentColor", strokeWidth: 1.2, strokeLinecap: "round", strokeLinejoin: "round" }));
		}
		...
		function NewJobForm({ t, workspaces, defaultCwd, onBack, onCreated }) {
		...
		*/
		function CronJobsAction({ wide, t, workspaces, useSessions, useWorkspaces }) {
			...
			const [detailFault, setDetailFault] = react.useState(null);
			...
			// The form's working-directory default — the agent's own directory:
			// the current session's cwd first, then the workspace holding that
			// session, then a lone workspace when nothing is current.
			...
			if (view === null && snapshot?.error !== undefined) {
				...
				body = h(react.Fragment, {},
					...
					h(NewJobForm, { t, workspaces, defaultCwd, onBack: () => setView(null), onCreated: () => {
						setView(null);
						refresh();
					} }));
			} else {
				...
				body = h(react.Fragment, {},
					h("div", { className: "dshCron_detailHead" },
						...
						job !== undefined && job.source === "manual" && !confirmDelete ? h("button", {
							type: "button",
							className: "dshCron_iconBtn",
						...
						job !== undefined && job.source === "manual" && confirmDelete ? h("button", {
							type: "button",
							className: "dshCron_delConfirm",
							onClick: async () => {
								const outcome = await postAction(DELETE_PATH, { job: jobName });
								setConfirmDelete(false);
								if (!outcome.ok) {
									setDetailFault(t("action.failed", { message: outcome.message }));
									refresh();
									return;
								}
								setView(null);
					...
					detailFault !== null ? h("p", { className: "dshCron_note dshCron_noteError", role: "alert", style: { paddingTop: 0 } }, detailFault) : null,
