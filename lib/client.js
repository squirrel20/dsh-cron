window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			"form.task": "任务",
			"form.prompt": "prompt",
			"form.prompt.hint": "固定 [CRON RUN] 无人值守 framing 经 system prompt 注入",
		...
		const en = {
			...
			"form.task": "Task",
			"form.prompt": "Prompt",
			"form.prompt.hint": "runs unattended; the fixed [CRON RUN] framing is injected via the system prompt",
		...
		*/
		...
		/**
		* The status-dot state of one job row. Job rows stay quiet in the
		* sidebar: only live (ongoing) and failed (error) states color the
		* dot; every settled outcome reads muted.
		*/
		function jobDotState(job) {
			if (job.running === true) return "ongoing";
			if (job.enabled === false) return "muted";
			if (job.lastRun !== undefined && statusMeta(job.lastRun.status).dot === "error") return "error";
			return "muted";
		}
		/**
		...
		*/
		function RunRow({ record, expanded, selected, nowMs, t, onOpenSession, onToggle }) {
			...
			// The dot alone carries the outcome, and quietly: like job rows,
			// only live (ongoing) and failed (error) color it. The right edge
			// says when the run started (the job row already ticks a live
			// elapsed).
			const line = h("span", { className: "dshCron_runLine" },
				h(Dot, { state: running ? "ongoing" : meta.dot === "error" ? "error" : "muted", size: 8 }),
			...
			return h("button", {
				...
				onClick: () => {
					...
					// back to the inline card instead of a dead click.
