window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			"div[data-slot=\"sidebar.workspaces\"]>*{flex:0 1 auto;min-height:0}",
			".dshCron_holder{box-sizing:border-box;flex:none;width:100%;min-width:0;display:flex;padding-right:var(--dsh-sidebar-inline-padding,12px)}",
			".dshCron_section{box-sizing:border-box;width:100%;min-width:0;flex:none;display:flex;flex-direction:column;margin:2px 0 4px}",
			...
			".dshCron_headToggle{display:flex;flex:1;min-width:0;align-items:center;gap:4px;background:0 0;border:0;padding:0;cursor:pointer;color:inherit;font:inherit;text-align:left}",
			".dshCron_headToggle:hover{color:var(--dsw-alias-label-secondary)}",
			".dshCron_headChevron{flex:none;display:inline-flex;color:var(--dsw-alias-label-caption)}",
			...
			".dshCron_whenDanger{color:var(--dsw-alias-state-error-primary)}",
			...
			".dshCron_card{margin-left:20px;background:var(--dsw-alias-markdown-code-block,var(--dsw-alias-interactive-bg-hover));border-radius:8px;padding:8px 10px;display:flex;flex-direction:column;gap:4px}",
			...
			".dshCron_mwrap{position:relative}",
			...
			".dshCron_form{display:flex;flex-direction:column;gap:10px;min-width:0}",
		...
		const zh = {
			...
			"section.add": "新建作业",
			"section.expand": "展开定时任务",
			"section.collapse": "收起定时任务",
		...
		const en = {
			...
			"section.add": "New job",
			"section.expand": "Expand cron jobs",
			"section.collapse": "Collapse cron jobs",
		...
		*/
		...
		/**
		* Fixed list order — no Group by / Order by control by design:
		* running first, then jobs whose last run failed, then by next fire
		* time ascending, then never-fired, disabled last; name breaks ties.
		*/
		...
		/** Sort jobs by the fixed rule; stable within a rank. */
		...
		/** The right-edge label of one job row: live duration, danger, or next fire. */
		function jobWhen(job, lastRecord, nowMs, t) {
			...
			const meta = job.lastRun !== undefined ? statusMeta(job.lastRun.status) : undefined;
			if (meta !== undefined && meta.danger === true) return { text: meta.key !== undefined ? t(meta.key) : meta.raw, tone: "danger" };
				...
				h("span", { className: "dshCron_slot" },
					...
					h("span", { className: "dshCron_slotArrow" },
						h("span", { className: expanded ? "dshCron_arrow dshCron_arrowOpen" : "dshCron_arrow" },
							h(IconChevron, { size: 12 })))),
				h("span", { className: muted ? "dshCron_name dshCron_nameMuted" : "dshCron_name", title: `${job.name} · ${job.schedule}` }, job.name),
				confirmDelete
					? h("button", {
						type: "button",
					...
					: h(react.Fragment, {},
						h("span", {
							className: when.tone === "ongoing" ? "dshCron_when dshCron_whenOngoing" : when.tone === "danger" ? "dshCron_when dshCron_whenDanger" : "dshCron_when",
							title: job.schedule,
						}, when.text),
		...
		* (command runs, pruned sessions) expands its output card inline.
		*/
		function RunRow({ record, expanded, selected, nowMs, t, onOpenSession, onToggle }) {
			const meta = statusMeta(record.status);
			const running = record.status === "running";
			const statusText = meta.key !== undefined ? t(meta.key) : meta.raw;
			const expandable = record.summary !== "" || record.error !== undefined || record.exitCode !== undefined;
			...
			const line = h("span", { className: "dshCron_runLine" },
				...
				h("span", { className: "dshCron_target" },
					formatInstant(record.target, t),
					record.manual === true ? ` · ${t("detail.manual")}` : ""),
				meta.danger === true || running
					? h("span", { className: running ? "dshCron_when dshCron_whenOngoing" : "dshCron_when dshCron_whenDanger" },
						running ? formatDuration(runElapsedMs(record, nowMs), t) : statusText)
					: h("span", { className: "dshCron_when" }, formatDuration(runElapsedMs(record, nowMs), t)));
			...
			if (!openable && !expandable) return h("div", { className: selected ? "dshCron_runRow dshCron_runSelected" : "dshCron_runRow" }, line);
			return h("button", {
				type: "button",
				className: selected ? "dshCron_runRow dshCron_runSelected" : "dshCron_runRow",
				"aria-expanded": expandable ? expanded : undefined,
				onClick: () => {
					if (openable) onOpenSession(record.sessionId);
					else onToggle();
				},
			}, line, card);
		}
		...
		*/
		function ModelChip({ t, options, provider, model, effort, onChange }) {
			...
			let panel = null;
			if (open && view === "root") {
				panel = h("div", { className: "dshCron_mpanel" },
		...
		*/
		function JobDialog({ t, workspaces, defaultCwd, initial, onClose, onSaved }) {
			...
			const [trigger, setTrigger] = react.useState(initial?.schedule?.cron !== undefined ? "cron" : initial?.schedule?.everySeconds !== undefined ? "every" : initial?.schedule?.at !== undefined ? "at" : "cron");
			const [cron, setCron] = react.useState(initial?.schedule?.cron ?? "0 7 * * *");
			...
			const [everySeconds, setEverySeconds] = react.useState(String(initial?.schedule?.everySeconds ?? 3600));
			const [at, setAt] = react.useState(initial?.schedule?.at !== undefined ? toLocalInput(initial.schedule.at) : "");
				...
				h(primitives.Modal, {
					...
					h("div", { className: "dshCron_form" },
						...
						h("div", { className: "dshCron_field" },
							...
							trigger === "every" ? h("div", { className: "dshCron_inputRow" },
								h("input", { className: "dshCron_input dshCron_mono", style: { flex: 1 }, type: "number", min: 60, value: everySeconds, "aria-label": t("form.every"), onChange: (event) => setEverySeconds(event.target.value) }),
						...
						taskKind === "agent" ? (() => {
							...
							const presetInherit = inheritLabel(options?.presets, options?.defaults?.preset);
							const accessInherit = inheritLabel(options?.access, options?.defaults?.access);
							const chosenPreset = options?.presets?.find((entry) => entry.id === preset);
							const chosenAccess = options?.access?.find((entry) => entry.id === access);
							return h(react.Fragment, {},
								field(t("form.preset"), h("select", {
									...
									...(options?.presets ?? []).map((entry) => h("option", { key: entry.id, value: entry.id, title: entry.description }, entry.name)))),
								field(t("form.access"), h("select", {
									className: "dshCron_input",
									value: access,
						...
						taskKind === "agent" ? h("div", { className: "dshCron_field" },
							h("span", { className: "dshCron_lbl" }, t("form.model")),
							h(ModelChip, {
								t,
								options,
		...
		const RUNS_PAGE = 5;
		/** localStorage key remembering the section's collapse state ("0" = expanded); absent = collapsed. */
		const COLLAPSE_KEY = "dsh-cron.collapsed";
		/**
		...
		*/
		function CronSection({ wide, t, workspaces, openSession, useSessions, useWorkspaces }) {
			...
			// Collapsed by default; only the explicit header toggle persists.
			...
			const jobs = react.useMemo(() => sortJobs(snapshot?.data?.jobs ?? []), [snapshot]);
			...
			if (!wide) return null;
			// An open search always shows the list, even from the collapsed state.
			const open = searchOpen || !collapsed;
			...
			const section = h("div", { className: "dshCron_section" },
				h("div", { className: "dshCron_head" },
					searchOpen
						? h("div", { className: "dshCron_searchBox" },
							h("button", {
								type: "button",
						...
						: h(react.Fragment, {},
							h("button", {
								type: "button",
								className: "dshCron_headToggle",
								"aria-expanded": open,
								"aria-label": t(open ? "section.collapse" : "section.expand"),
								onClick: toggleCollapsed,
							},
								h("span", { className: "dshCron_headLabel" }, t("section.title")),
								h("span", { className: "dshCron_headChevron" },
									h("span", { className: open ? "dshCron_arrow dshCron_arrowOpen" : "dshCron_arrow" },
										h(IconChevron, { size: 12 }))),
				...
				open && actionFault !== null ? h("p", { className: "dshCron_note dshCron_noteError", role: "alert", style: { paddingTop: 0, paddingBottom: 2 } }, actionFault) : null,
				open ? h("div", { className: "dshCron_list" }, body) : null,
