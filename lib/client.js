window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//#region icons
		// dsh 0.1.7 renamed the primitives icon set from Icon<Name><Size> to
		// Icon<Name>Regular|Medium (#5). Resolve the new name first, then the
		// legacy one, then an empty glyph, so a future rename costs an icon
		// instead of crashing the whole slot entry (React #130).
		const NoIcon = () => null;
		const icon = (stem, legacySize) =>
			primitives[`Icon${stem}Regular`] ?? primitives[`Icon${stem}${legacySize}`] ?? NoIcon;
		const IconPlusOutline = icon("PlusOutline", "16");
		const IconTrashOutline = icon("TrashOutline", "16");
		const IconEllipsisOutline = icon("EllipsisOutline", "16");
		const IconEditOutline = icon("EditOutline", "16");
		const IconPauseOutline = icon("PauseOutline", "16");
		const IconPlayOutline = icon("PlayOutline", "16");
		const IconSearchOutline = icon("SearchOutline", "16");
		const IconCloseOutline = icon("CloseOutline", "16");
		const IconPersonalizationOutline = icon("PersonalizationOutline", "16");
		const IconStopFill = icon("StopFill", "16");
		const IconTriangleRightFill = icon("TriangleRightFill", "14");
		//#endregion
		//#region styles
		...
		const css = [
			...
			// The header title doubles as the section's collapse control: the same
			// triangle the job rows use, rotated 90deg while the section is open.
			"button.dshCron_headTitle{height:28px;padding:0;border:0;background:0 0;color:inherit;font:inherit;text-align:inherit;cursor:pointer;border-radius:6px}",
			"button.dshCron_headTitle:hover .dshCron_headLabel{color:var(--dsw-alias-label-secondary)}",
			".dshCron_headArrow{flex:none;width:14px;display:inline-flex;align-items:center;justify-content:center;color:var(--dsw-alias-label-caption);transition:transform .15s var(--ds-ease-in-out)}",
			".dshCron_headArrowOpen{transform:rotate(90deg)}",
			...
			// command-run detail page: a shell.overlay entry (root-scoped list
			// slot), the same layer the ingest source page uses. The layer is
			// inset:0 over the whole frame, so the page leaves the sidebar
			// column free itself: the column is drag-resizable, so its width is
			// measured live into --dshCron-left (see useSidebarInset).
			".dshCron_page{box-sizing:border-box;position:absolute;top:0;bottom:0;right:0;left:var(--dshCron-left,280px);display:flex;flex-direction:column;min-width:0;background:var(--dsw-alias-bg-base);border-left:1px solid var(--dsw-alias-border-l1)}",
			...
			"@media (prefers-reduced-motion:reduce){.dshCron_arrow,.dshCron_headArrow{transition:none}}",
		].join("");
		...
		*/
		...
		/** localStorage slot for the section's collapsed state. */
		const COLLAPSE_STORE = "dshCron.collapsed";
		...
		*/
		function jobWhen(job, lastRecord, nowMs, t) {
			// An orphan cannot fire at all: its provider plugin is not mounted,
			// so what is left is history — say that instead of "disabled",
			// which would suggest a switch the user could flip back.
			if (job.orphan === true) return { text: t("status.orphan"), tone: "muted" };
			if (job.running === true) {
		...
		*/
		function JobRow({ job, expanded, lastRecord, nowMs, confirmDelete, t, onToggle, onMenu, onConfirmDelete }) {
			...
			const orphan = job.orphan === true;
			// An orphan has no live job behind it — running, pausing or editing
			// would all fail server-side, so the only honest action is clearing
			// the history it left.
			const items = orphan
				? [{ id: "delete", label: t("menu.delete"), icon: h(IconTrashOutline, { size: 16 }), danger: true }]
				: [
					job.running === true
						? { id: "stop", label: t("menu.stop"), icon: h(IconStopFill, { size: 16 }) }
						: { id: "run", label: t("menu.run"), icon: h(IconPlayOutline, { size: 16 }) },
					job.enabled === false
						? { id: "resume", label: t("menu.resume"), icon: h(IconPlayOutline, { size: 16 }) }
						: { id: "pause", label: t("menu.pause"), icon: h(IconPauseOutline, { size: 16 }) },
					...manual ? [
						{ id: "edit", label: t("menu.edit"), icon: h(IconEditOutline, { size: 16 }) },
						{ id: "delete", label: t("menu.delete"), icon: h(IconTrashOutline, { size: 16 }), danger: true },
					] : [],
				];
			return h("div", {
				...
				h("span", { className: "dshCron_slot" },
					...
					h("span", { className: "dshCron_slotArrow" },
						h("span", { className: expanded ? "dshCron_arrow dshCron_arrowOpen" : "dshCron_arrow" },
							h(IconTriangleRightFill, {})))),
				h("span", {
					className: muted ? "dshCron_name dshCron_nameMuted" : "dshCron_name",
					// The owner is part of the row's identity for plugin jobs:
					// it is the answer to "where did this job come from, and
					// where do I change it".
					title: `${job.name}${job.description !== undefined ? ` · ${job.description}` : ""} · ${job.schedule}`
						+ (job.owner === undefined ? "" : ` · ${job.owner}`),
				}, job.name),
				confirmDelete
					? h("button", {
						type: "button",
		...
		*/
		function RunRow({ record, selected, nowMs, t, onOpenSession, onOpenPage }) {
			...
			return h("button", {
				...
				onClick: async () => {
					// sessions.open throws for a session the client does not list
					// (stale list after a host restart, GC-pruned session, or a
					// settled run whose plain Session left the list when the host
					// disposed its Agent). openSession refreshes the host list once
					// and retries; only an id that stays unknown falls back to the
					// detail page instead of a dead click.
					if (openable && await onOpenSession(record.sessionId)) return;
					onOpenPage(record);
				},
			}, line);
		}
		/**
		* The command-run detail page. Rendered into shell.overlay (see
		* RunPageOverlay), covering the main area; shows the settled (or live)
		* record of one run: status, timing, exit code, the job's argv/cwd when
		* the manual spec is at hand, and the output tail. While the run is in
		* flight the record's summary carries the live output tail (spliced in
		* server-side) and the section polls fast, so the output streams here;
		* the body follows the tail unless the reader scrolls away from the
		* bottom. `left` is the measured sidebar width the page must leave free.
		*/
		function CommandRunPage({ record, spec, nowMs, left, t, onClose }) {
			const meta = statusMeta(record.status);
			const running = record.status === "running";
			...
			return h("div", {
				className: "dshCron_page",
				style: { "--dshCron-left": `${left}px` },
				role: "region",
				"aria-label": `${record.job} #${record.seq}`,
			},
		...
		*/
		...
		function ViewOptionsMenu({ orderBy, onPick, t }) {
			...
			return h(primitives.Menu, {
				...
				}, h(IconPersonalizationOutline, {})),
			});
		}
		//#region run page store
		/**
		* Page state lives outside React.
		*
		* The sidebar section and the run-detail page are two separate slot
		* entries (sidebar.footer.action and shell.overlay) mounted on different
		* host render trees, with no common ancestor to hang a Context on — so
		* "click a row on the left, open that page on the right" has to be held
		* by one module-level store. The section stays the only place that polls
		* /state; it pushes the resolved record in (see setPageData).
		*/
		const pageState = {
			/** { job, seq, session } of the open run; null = no page. */
			open: null,
			/** Record + spec pushed in by the section, re-derived per snapshot. */
			record: undefined,
			spec: undefined,
			version: 0,
			listeners: new Set(),
		...
		function emitPage() {
			pageState.version += 1;
			for (const listener of [...pageState.listeners]) listener();
		}
		function subscribePage(listener) {
			pageState.listeners.add(listener);
			return () => pageState.listeners.delete(listener);
		}
		/** Subscribe to the store; re-render is driven by the version scalar. */
		function usePageStore() {
			react.useSyncExternalStore(subscribePage, () => pageState.version, () => pageState.version);
			return pageState;
		}
		/**
		* One page per overlay layer.
		*
		* shell.overlay is a list slot: entries open independently, so two
		* plugins' pages stack — the later one paints over the earlier, and the
		* buried one reads as "the click did nothing". Each page announces its
		* own opening; everyone else closes on hearing someone else's id. A
		* window event is the smallest convention that needs no cross-plugin
		* import (dsh-ingest implements the same one).
		*/
		const OVERLAY_EVENT = "dsh-overlay:open";
		const OVERLAY_ID = "cron-run-page";
		function openRunPage(target) {
			pageState.open = target;
			window.dispatchEvent(new CustomEvent(OVERLAY_EVENT, { detail: { id: OVERLAY_ID } }));
			emitPage();
		}
		function closeRunPage() {
			if (pageState.open === null) return;
			pageState.open = null;
			pageState.record = undefined;
			pageState.spec = undefined;
			emitPage();
		}
		/** The section pushes what it resolved out of the current snapshot. */
		function setPageData(record, spec) {
			if (pageState.record === record && pageState.spec === spec) return;
			pageState.record = record;
			pageState.spec = spec;
			emitPage();
		}
		/**
		* Measure the sidebar column into --dshCron-left.
		*
		* The overlay layer is inset:0 over the whole frame (sidebar included),
		* so the page has to leave that column free itself. The width is
		* draggable (and collapses to a rail on narrow viewports), so a hard
		* 280 drifts out of alignment: find the frame back from
		* [data-shell-overlay], take its first grid child (sidebarCol) and track
		* it with a ResizeObserver. Both anchors are host structure contracts
		* (attribute + grid order), steadier than hashed CSS Module names.
		*/
		function useSidebarInset(open) {
			const [left, setLeft] = react.useState(280);
			react.useEffect(() => {
				if (!open) return;
				const layer = document.querySelector("[data-shell-overlay]");
				const column = layer?.parentElement?.firstElementChild ?? null;
				if (column === null) return;
				const measure = () => setLeft(Math.round(column.getBoundingClientRect().width));
				measure();
		...
		/**
		* The shell.overlay entry. Data arrives from the section through the
		* store (the section polls /state and keeps running even in rail mode,
		* where it renders nothing); this only renders and ticks its own clock
		* while the run is in flight.
		*/
		function RunPageOverlay({ t }) {
			const state = usePageStore();
			const record = state.open === null ? undefined : state.record;
			const left = useSidebarInset(record !== undefined);
			const running = record?.status === "running";
			...
			return h(CommandRunPage, { record, spec: state.spec, nowMs, left, t, onClose: closeRunPage });
		}
		//#endregion
		...
		*/
		function CronSection({ wide, t, workspaces, openSession, useSessions, useWorkspaces }) {
			...
			// The collapsed state persists the same way the Order by choice does:
			// a storage fault degrades to "expanded", never to a broken section.
			const [collapsed, setCollapsedState] = react.useState(() => {
				try {
					return localStorage.getItem(COLLAPSE_STORE) === "1";
				} catch {
					return false;
				}
			});
			...
			// The page itself lives in the shell.overlay layer (RunPageOverlay);
			// what stays here is dismissal. Escape closes it, and so does a
			// press on a sidebar session row: opening the session that is
			// already current moves nothing in the sessions store, so the
			// current-session effect below never fires for it. role=treeitem
			// with aria-selected is exactly the rows that open a session
			// (session rows and search hits) — project rows carry aria-expanded
			// instead. A press on a row's action button is not navigation and
			// leaves the page alone.
			const pageOpen = page.open !== null;
			react.useEffect(() => {
				if (!pageOpen) return;
				const onKey = (event) => {
					if (event.key === "Escape") closeRunPage();
				};
			...
			const pageRecord = page.open === null ? undefined : runs.find((record) => record.job === page.open.job && record.seq === page.open.seq);
			react.useEffect(() => {
				if (page.open !== null && snapshot?.data !== undefined && pageRecord === undefined) closeRunPage();
			}, [page.open, snapshot, pageRecord]);
			// The section owns the data plane; the overlay page only renders.
			react.useEffect(() => {
				if (page.open === null) return;
				setPageData(pageRecord, pageRecord === undefined ? undefined : specs[pageRecord.job]);
			}, [page.open, pageRecord, specs]);
			...
			react.useEffect(() => {
				if (page.open === null || page.open.session === currentSession) return;
				closeRunPage();
			}, [page.open, currentSession]);
					...
					return [
						...
						...shown.map((record) => h(RunRow, {
							...
							selected: (record.sessionId !== undefined && record.sessionId === currentSession)
								|| (page.open !== null && page.open.job === record.job && page.open.seq === record.seq),
							nowMs,
							t,
							onOpenSession: async (sessionId) => {
								const opened = await openSession(sessionId);
								if (opened) closeRunPage();
								return opened;
							},
							onOpenPage: (target) => {
								if (page.open !== null && page.open.job === target.job && page.open.seq === target.seq) closeRunPage();
								else openRunPage({ job: target.job, seq: target.seq, session: currentSession });
							},
						})),
					...
					searchOpen
						? h("div", {
							...
							query !== "" ? h("button", {
								...
								onMouseDown: (event) => event.preventDefault(),
								onClick: () => setQuery(""),
							}, h(IconCloseOutline, { size: 14 })) : null)
						: h(react.Fragment, {},
							h("button", {
								type: "button",
								className: "dshCron_headTitle",
								"aria-expanded": !collapsed,
								title: collapsed ? t("section.expand") : t("section.collapse"),
								onClick: () => setCollapsed(!collapsed),
							},
								h("span", { className: collapsed ? "dshCron_headArrow" : "dshCron_headArrow dshCron_headArrowOpen" },
									h(IconTriangleRightFill, {})),
							...
							h("button", {
								...
								// Searching or adding inside a collapsed section would land
								// out of sight; both open the list first.
								onClick: () => {
									setCollapsed(false);
									setSearchOpen(true);
								},
							...
							}, h(IconPlusOutline, { size: 16 })))),
				collapsed ? null : actionFault !== null ? h("p", { className: "dshCron_note dshCron_noteError", role: "alert", style: { paddingTop: 0, paddingBottom: 2 } }, actionFault) : null,
				collapsed ? null : h("div", { className: "dshCron_list" }, body),
		...
		/**
		* Register dictionaries, the sidebar section (foot slot, above settings)
		* and the run-detail page (shell.overlay).
		*/
		function apply(ctx) {
			ctx.effect(() => ctx.locale.register(NS, { zh, en }), "cron: dictionaries");
			// Another plugin opening its own overlay page closes this one, or
			// the two stack and whichever paints on top swallows the other's
			// clicks. See the OVERLAY_EVENT note on the page store.
			ctx.effect(() => {
				const onOpen = (event) => {
					if (event.detail?.id === OVERLAY_ID) return;
					closeRunPage();
				};
				window.addEventListener(OVERLAY_EVENT, onOpen);
				return () => window.removeEventListener(OVERLAY_EVENT, onOpen);
			}, "cron: close run page when another overlay opens");
			ctx.slots.inject("sidebar.footer.action", () => ctx.slots.register({
				...
				inject: () => ({
					...
					// A run's plain Session drops out of the client list the moment
					// the host disposes the run's Agent, while its log stays
					// persisted: re-pull the authoritative list and retry, so the
					// run row still opens the finished conversation.
					openSession: async (sessionId) => {
						try {
							...
							try {
								...
								console.warn("cron: open session failed:", retryError);
								return false;
							}
						}
					},
