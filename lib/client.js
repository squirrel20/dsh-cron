window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		* the job's argv/cwd when the manual spec is at hand, and the stored
		* output tail. Content mirrors what the run record actually holds —
		* command output streams nowhere, so a running record shows timing only.
		*/
		function CommandRunPage({ record, spec, nowMs, t, onClose }) {
			...
			return h("div", { className: "dshCron_page", role: "region", "aria-label": `${record.job} #${record.seq}` },
				...
				h("div", { className: "dshCron_pageBody" },
					h("div", { className: "dshCron_pageInner" },
						...
						h("div", {},
							...
							h("pre", { className: "dshCron_pagePre" }, record.summary !== "" ? record.summary : t("page.empty"))))));
		}
