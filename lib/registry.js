 ...
 */
import { PLUGIN_NORMALIZE, normalizeJobs } from "./service.js";
...
/** A `callback` task registered without the function it is supposed to run. */
export class MissingHandlerError extends Error {
	constructor(name) {
		super(`cron job '${name}': a callback task needs options.run — the function one occurrence executes`);
		this.code = "missing_handler";
	}
}
...
export class JobRegistry {
	...
	#attach(name) {
		return this.#enqueue(`attaching plugin job '${name}'`, async () => {
			...
			await service.attachPluginJob(entry.spec, entry.owner, entry.run);
		});
	}
	 ...
	 * and is validated by the same {@link normalizeJobs} vocabulary, plus one
	 * kind only a plugin can use: `task.kind: "callback"`, whose work is the
	 * `options.run` function called in process. `enabled` is only the initial
	 * value: a user pausing the job in the overlay writes a durable override
	 * that outlives re-registration.
	 *
	 * @param options.run - the handler for a `callback` task, receiving
	 *   `{ job, target, seq, signal }` and returning a summary string or
	 *   `{ ok?, summary?, error? }`. Never persisted.
	...
	register(spec, options = {}) {
		...
		const job = normalizeJobs([spec], PLUGIN_NORMALIZE)[0];
		if (job.task.kind === "callback" && typeof options.run !== "function") {
			throw new MissingHandlerError(job.name);
		}
		const name = job.name;
		...
		this.entries.set(name, {
			...
			owner,
			...(typeof options.run === "function" ? { run: options.run } : {}),
		});
		this.#attach(name);
